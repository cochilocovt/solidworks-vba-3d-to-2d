...
Public Function ImportModelItemsAcrossDrawing( _
    ...
    If total = 0 Then
        evidence.AddWarning _
            "Model-item import returned zero; fallback can locate " & _
            "owned holes but cannot invent missing size, fit, tolerance, " & _
                "or GD&T intent."
    End If
        ...
        If Module8_RuntimeSupport.IsOrdinateEligibleView(swView) Then
            ...
            If activeView Is Nothing Or StrComp( _
                ...
                evidence.AddFailure _
                    "Orthographic model import: active-view " & _
            ...
            Else
                evidence.AddWarning _
                    "Orthographic model import: SelectByID2 " & _
            ...
            evidence.AddInfo "Orthographic selected-view import '" & _
                viewName & _
                "' count=" & CStr(insertedCount) & "; mask=" & CStr(mask) & "."
        Else
            evidence.AddInfo _
                "Model import skipped unsupported or unregistered view '" & _
                viewName & "'."
        End If
...
Public Function ApplyImportedCoverageToCandidates( _
    ...
    If Not horizontalDatum.Proven Or horizontalDatum.DrawingEntity Is _
        Nothing Or _
    ...
    If Not IsArray(dimensions) Then
        evidence.AddFailure _
            "Coverage inspection could not enumerate display " & _
            "dimensions in '" & _
                    ...
                    Case swDimensionType_HorOrdinate
                        ...
                        ElseIf AttachmentsMatchAnyCandidate( _
                            ...
                            evidence.AddFailure _
                                ...
                                Module8_RuntimeSupport.GetViewName(swView) & _
                                "' matches an owned candidate but not the " & _
                                    "approved X datum."
                            Exit Function
                        End If
                    ...
                    Case swDimensionType_HorLinear
                        ...
                        ElseIf AttachmentsMatchAnyCandidate( _
                            ...
                            evidence.AddFailure _
                                ...
                                Module8_RuntimeSupport.GetViewName(swView) & _
                                "' matches an owned candidate but its X " & _
                                    "datum is unresolved."
                            Exit Function
                        End If
                    ...
                    Case swDimensionType_VertLinear
                        ...
                        ElseIf AttachmentsMatchAnyCandidate( _
                            ...
                            evidence.AddFailure _
                                ...
                                Module8_RuntimeSupport.GetViewName(swView) & _
                                "' matches an owned candidate but its Y " & _
                                    "datum is unresolved."
                            Exit Function
                        End If
                End Select
...
Failed:
    evidence.AddFailure _
        "Attachment inspection failed for location dimension '" & _
...
Private Function IsLocationDimensionType(ByVal dimensionType As Long) As _
    Boolean
    Select Case dimensionType
...
Public Sub AutoArrangeAllDrawingDimensions( _
    ...
    Do While Not swView Is Nothing
        ...
        ' R23-809. Section dimensions use semantic lanes. The generic
        ' arranger cannot see those lanes, so moving them here would undo
        ' the section-dimension engine's deliberate placement.
        If Module10_SectionDimensionEngine.IsExcludedFromGenericArrangement( _
            swView) Then

            excludedViews = excludedViews + 1
            evidence.AddInfo "DIMENSION_ARRANGE_SKIPPED|view=" & _
                EvidenceValue(Module8_RuntimeSupport.GetViewName(swView)) & _
                "|reason=SectionSemanticLanes"
            GoTo ContinueView
        End If
...
ContinueView:
    ...
    stageDetail = "views=" & CStr(inspectedViews) & _
        ...
        ", noAction=" & CStr(noActionViews) & _
        ", excluded=" & CStr(excludedViews)
...
Private Function AutoArrangeDimensionsInView( _
    ...
    If Not Module8_RuntimeSupport.ActivateDrawingView( _
        ...
        evidence.AddFailure _
            "Dimension arrange could not prove active view '" & _
            viewName & "'."
    ...
    If Not IsArray(dimensions) Then
        evidence.AddFailure _
            "Dimension arrange could not enumerate display " & _
    ...
    If selectionManager Is Nothing Then
        evidence.AddFailure _
            "Dimension arrange selection manager is Nothing in '" & _
            viewName & "'."
    ...
    If selectData Is Nothing Then
        ...
        GoTo SafeExit
    End If
    ' ISelectData.View is not settable in every drawing context on this
    ' build: it raises 91 here, which is why Modules 13, 15 and 16 each wrap
    ' the same assignment. Module4 assigned it raw, so every arrange attempt
    ' died in the handler - reported as "arrange API error ... 0:" until the
    ' r40 capture fix exposed the real number. Binding is an optimisation for
    ' scoping the selection, not a precondition: Select3 still receives the
    ' SelectData object and the view has already been activated and read
    ' back, so an unbound view is recorded and the transaction continues.
    Dim viewBinding As String
    viewBinding = TryBindSelectDataView(selectData, swView)

    evidence.AddInfo "DIMENSION_ARRANGE_BINDING|view=" & _
        EvidenceValue(viewName) & "|viewBinding=" & viewBinding
    ...
    For i = LBound(dimensions) To UBound(dimensions)
        ...
        If displayDimension Is Nothing Then
            evidence.AddFailure _
                "Dimension arrange received a Nothing display " & _
                "dimension in '" & viewName & "' at array index " & CStr(i) _
                    & "."
        ...
        If annotation Is Nothing Then
            evidence.AddFailure _
                "Dimension arrange received a display dimension " & _
                "without an annotation in '" & viewName & _
                    "' at array index " & _
                CStr(i) & "."
    ...
    If readbackCount <> selectedCount Then
        evidence.AddWarning _
            "Dimension arrange selection readback differs from " & _
...
Failed:
    ' Capture before anything else runs. VBA evaluates a concatenation left
    ' to right, and GetViewName contains On Error Resume Next, which resets
    ' the global Err object. Reading Err.Number after that call is why the
    ' 18:45 and 23:24 runs both reported "arrange API error ... : 0: " with
    ' no number and no description, hiding a live failure on two views.
    Dim arrangeErrorNumber As Long
    Dim arrangeErrorDescription As String
    arrangeErrorNumber = Err.Number
    arrangeErrorDescription = Err.Description

    evidence.AddFailure "Dimension arrange API error in '" & _
        Module8_RuntimeSupport.GetViewName(swView) & "': " & _
        CStr(arrangeErrorNumber) & ": " & arrangeErrorDescription
    arrangeOutcome = "FAILED_API_ERROR"
    Resume SafeExit
End Function

' Mirrors the identical helper in Modules 13, 15 and 16. Setting
' ISelectData.View raises 91 in some drawing contexts on this build, so the
' result is reported as evidence rather than allowed to abort the caller.
...
Private Function TryArrangeWithDeterministicLanes( _
    ...
    If Not TryReadViewOutline( _
        ...
        evidence.AddFailure _
            "Dimension lane fallback could not read a valid " & _
    ...
    If Not ContentBorderIsValid(evidence) Then
        evidence.AddFailure _
            "Dimension lane fallback has no proved content " & _
            "border for '" & viewName & "'."
        Exit Function
    End If
    ...
    For i = LBound(dimensions) To UBound(dimensions)
        ...
        If Not TryReadAnnotationPosition( _
            ...
            evidence.AddFailure _
                "Dimension lane fallback could not read the " & _
        ...
        ElseIf DimensionPositionIsUnsupported(dimensionType) Then
            If Not PositionIsInsideContentBorder( _
                ...
                evidence.AddFailure _
                    "Unmovable radial or diametric dimension " & _
        ...
        Else
            ...
            If Not TryChooseDimensionLane( _
                ...
                evidence.AddFailure _
                    "Dimension lane fallback could not allocate " & _
                    "a border-safe lane in '" & viewName & _
                        "' at array index " & _
                    CStr(i) & "."
                Exit Function
            End If
            ...
            If Not positionSet Then
                evidence.AddFailure _
                    "IAnnotation.SetPosition2 returned False " & _
            ...
            If Not TryReadAnnotationPosition( _
                ...
                evidence.AddFailure _
                    "Dimension lane fallback position readback " & _
            ...
            If Not PositionIsInsideContentBorder( _
                ...
                evidence.AddFailure _
                    "Dimension lane fallback readback is outside " & _
                    "the content border in '" & viewName & _
                        "' at array index " & _
                    CStr(i) & "."
                Exit Function
            End If
    ...
    If arrangedCount = 0 Then
        evidence.AddFailure _
            "Dimension fallback could not arrange any of the " & _
            "selected dimensions in '" & viewName & _
                "'; retained unsupported=" & _
...
Private Function TryAutoJogOrdinateDimension( _
    ...
    If Not autoJogSucceeded Then
        evidence.AddFailure _
            "IDisplayDimension.AutoJogOrdinate returned False " & _
    ...
    If Not TryReadAnnotationPosition( _
        ...
        evidence.AddFailure _
            "Ordinate auto-jog position readback failed in '" & _
...
Private Function ValidateDimensionAnnotationOrigins( _
    ...
    If Not ContentBorderIsValid(evidence) Then
        evidence.AddFailure _
            "Dimension arrange has no proved content border " & _
            "for '" & viewName & "'."
        Exit Function
    End If
    ...
    For i = LBound(dimensions) To UBound(dimensions)
        ...
        If Not TryReadAnnotationPosition(annotation, x, y) Then
            evidence.AddFailure _
                "Dimension arrange could not read annotation " & _
                "position in '" & viewName & "' at array index " & CStr(i) _
                    & "."
            Exit Function
        End If
        ...
        If Not PositionIsInsideContentBorder(x, y, evidence) Then
            evidence.AddFailure _
                "Dimension annotation origin is outside the " & _
...
Failed:
    evidence.AddFailure _
        "Dimension arrange position validation API error in '" & _
