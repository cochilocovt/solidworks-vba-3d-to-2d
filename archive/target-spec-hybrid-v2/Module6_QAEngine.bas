...
Public Sub PerformFinalDrawingChecks( _
    ...
    RequireStructuralQaStages evidence
    ...
    If Module1_Main.GlobalConfig.PopulateTitle And _
       evidence.TitlePropertiesWritten = 0 Then
        ...
        evidence.AddFailure _
            "QA: title population requested but no drawing " & _
    ...
    If Not evidence.LayoutBoundariesProven Then
        evidence.AddFailure _
            "QA: border/title-block layout boundaries were not proven."
        finalQaValid = False
        annotationExtentsValid = False
    End If
    ...
    If Not evidence.SheetScaleReadbackProven Then
        evidence.AddFailure _
            "QA: actual sheet scale was not read back and proved."
        finalQaValid = False
    End If
    ...
    If viewPolicyValid Then
        evidence.MarkStageProved "VIEW_DIMENSION_POLICY", _
            "isometric views have no dimensions and unsupported views " & _
                "have no ordinates"
    Else
        evidence.MarkStageFailed "VIEW_DIMENSION_POLICY", _
    ...
    If annotationExtentsValid Then
        evidence.MarkStageProved "ANNOTATION_EXTENTS", _
            "position-capable annotation origins, measurable note " & _
                "extents, and leader geometry passed"
    Else
        evidence.MarkStageFailed "ANNOTATION_EXTENTS", _
            "one or more annotation origins, measurable note extents, or " & _
                "leader paths were invalid"
    End If
    ...
    evidence.AddWarning _
        "Exact display-dimension glyph collision and manufacturing " & _
        "completeness still require visual comparison with the manual " & _
            "reference."
...
Private Sub RequireStructuralQaStages(ByRef evidence As CRunEvidence)
...
Private Function IsOrdinateDimensionType(ByVal dimensionType As Long) As _
    Boolean
    ...
    For pointIndex = firstIndex To UBound(leaderPoints) Step 3
        ...
        If hasPrevious Then
            If SegmentIntersectsRectangle( _
                ...
                evidence.AddFailure _
                    "Leader segment crosses the controlled " & _
                    "title block in '" & _
                    Module8_RuntimeSupport.GetViewName(swView) & "'."
                CheckLeaderPoints = False
            End If
...
Private Function CheckModelViewNoteExtent( _
    ...
    If extentLeft < evidence.ContentBorderLeft - BOUNDARY_TOLERANCE_M Or _
       ...
       extentTop > evidence.ContentBorderTop + BOUNDARY_TOLERANCE_M Then
        ...
        evidence.AddFailure _
            "Note extent crosses the measured zoned border in '" & _
            Module8_RuntimeSupport.GetViewName(swView) & "': " & _
    ...
    If RectanglesOverlap( _
        ...
        evidence.AddFailure _
            "Note extent intrudes into the measured title-block " & _
            "rectangle in '" & _
            Module8_RuntimeSupport.GetViewName(swView) & "'."
        Exit Function
    End If
...
Private Function CheckSectionLineClearance( _
    ...
    If Not evidence.PartIdentificationBoundsProven Then
        evidence.AddFailure _
            "P-0251 section clearance cannot be checked because " & _
    ...
    Do While Not swView Is Nothing
        ...
        If viewSectionLineCount > 0 Then
            If sectionLineInfoSize < 1 Then
                evidence.AddFailure _
                    "P-0251 section-line count is nonzero but " & _
                    "IView.GetSectionLineCount2 returned an invalid data " & _
                        "size " & _
    ...
    If sectionLineCount = 0 Then
        evidence.AddFailure _
            "P-0251 J-J section-line geometry readback is empty."
        GoTo ClearanceFailed
    End If

    evidence.MarkStageProved "SECTION_CLEARANCE", _
        CStr(sectionLineCount) & _
        " section line(s) proved clear of the measured " & _
            "part-identification extent"
    Exit Function

ClearanceFailed:
    evidence.MarkStageFailed "SECTION_CLEARANCE", _
        "section segment, arrow, or label geometry was unavailable or " & _
            "intersected the part-identification extent"
    ...
    For lineIndex = 1 To lineCount
        ...
        For segmentIndex = 1 To segmentCount
            ...
            If SectionSegmentTouchesPartIdentification( _
                ...
                evidence.AddFailure _
                    "P-0251 J-J section segment intersects " & _
        ...
        If SectionSegmentTouchesPartIdentification( _
               arrow1StartX, arrow1StartY, arrow1EndX, arrow1EndY, _
                   evidence) Or _
           SectionSegmentTouchesPartIdentification( _
               arrow2StartX, arrow2StartY, arrow2EndX, arrow2EndY, _
                   evidence) Then
...
Private Sub RecordSectionParseFailure( _
    ...
    evidence.AddFailure _
        "P-0251 section-line geometry could not be parsed in '" & _
...
Private Function ApplyFixtureSpecificChecks( _
    ...
    If ordinateEligibleCount < 2 Then
        evidence.AddFailure _
            "Fixture QA: fewer than two supported orthographic views."
        ApplyFixtureSpecificChecks = False
    End If

    If isometricCount < 1 Then
        evidence.AddFailure _
            "Fixture QA: required undimensioned isometric view is missing."
        ApplyFixtureSpecificChecks = False
    End If
    ...
    Select Case fixtureName
        Case "P-0251-14A-001"
            If sectionCount <> 1 Then
                evidence.AddFailure _
                    "P-0251 acceptance requires exactly one J-J section."
                ApplyFixtureSpecificChecks = False
            End If

            If Not evidence.StageIsProved( _
                Module19_SemanticQA.STAGE_MANUFACTURING_DEFINITION) Then

                evidence.AddFailure "P-0251 acceptance requires complete " & _
                    "source-backed associative manufacturing definitions."
                ApplyFixtureSpecificChecks = False
            End If
        ...
        Case "P-0252-01-001"
            If sectionCount > 0 Then
                evidence.AddWarning _
                    "P-0252-01-001 reference has no default section; " & _
        ...
        Case "P-0252-01-013"
            If sectionCount <> 1 Then
                evidence.AddFailure _
                    "P-0252-01-013 acceptance requires exactly one B-B " & _
                    "section."
                ApplyFixtureSpecificChecks = False
            End If
            ...
            evidence.AddWarning "P-0252-01-013 datum-symbol semantics are " & _
                "outside the current R23 graph gate and require visual review."
    End Select
End Function
    ...
    If Module1_Main.DIAGNOSTIC_DRAWING_MODE And _
       evidence.HasFailures And evidence.ViewsCreated > 0 Then
        ...
        MsgBox "Diagnostic drawing generation completed with " & _
            CStr(evidence.ViewsCreated) & " created view(s)." & vbCrLf & _
                vbCrLf & _
            "Production QA remains FAIL. Inspect the drawing and send the " & _
            "Immediate Window log or QA report for the remaining " & _
                "feature-level " & _
...
Private Sub RequireCoreStages(ByRef evidence As CRunEvidence)
    ...
    Module19_SemanticQA.RequireSemanticStages evidence
...
Private Function WriteEvidenceFileAtomically( _
    ...
    evidence.MarkStageProved "EVIDENCE_WRITE", _
        "Unicode temporary write, exact readback, atomic rename, and " & _
    ...
    If StrComp(readback, summary, vbBinaryCompare) <> 0 Then
        Err.Raise vbObjectError + 1602, _
            "WriteEvidenceFileAtomically", _
            "Temporary evidence readback did not exactly match the report " & _
                "text."
    End If
...
Private Function DirectoryEntryExists(ByVal path As String) As Boolean
    On Error Resume Next
    DirectoryEntryExists = (Len(Dir$(path, vbNormal Or vbHidden Or _
        vbSystem)) > 0)
    On Error GoTo 0
End Function
