...
Public Const MACRO_SOURCE_REVISION As String = _
    "target-spec-hybrid-v2-2026-08-05-r62"

' User decision, 2026-08-04. Preserve the approved drawing layout as-is.
' The production route must not reposition or rescale completed content.
' FINAL_LAYOUT remains reported as a user-accepted waiver, never as an
' automatic-clearance result.
Public Const R23_LAYOUT_USER_ACCEPTED_AS_IS As Boolean = True
...
Public Sub main()
    ...
    If Len(templatePath) = 0 Then
        GlobalEvidence.AddFailure _
            "Controlled drawing template is missing: " & _
...
Private Sub InitializeEvidence( _
    ...
    With GlobalEvidence
        ...
        .AddInfo _
            "Fixed workflow: model annotations, ownership-proven fallback " & _
            "ordinates, layout, title data, QA."
        .AddInfo _
            "Feature tolerances are recorded in Module8_RuntimeSupport " & _
            "and require runtime evidence."
    End With
End Sub
...
Private Function FileExists(ByVal path As String) As Boolean
    ...
    On Error Resume Next
    FileExists = (Len(Dir$(path, vbNormal Or vbReadOnly Or vbHidden Or _
        vbSystem)) > 0)
    On Error GoTo 0
End Function
