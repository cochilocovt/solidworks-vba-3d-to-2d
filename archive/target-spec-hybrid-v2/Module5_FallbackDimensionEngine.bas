...
Private Sub AddMissingOrdinateDimensionsToView( _
    ...
    If candidates.Count = 0 Then
        evidence.AddInfo _
            "No ownership-proven normal-axis hole centres in '" & _
            Module8_RuntimeSupport.GetViewName(swView) & "'."
        GoTo SafeExit
    End If
    ...
    If Not DatumIsUsable(horizontalDatum) Then
        RecordDatumFailure swView, "X", horizontalOrigin, horizontalDatum, _
            evidence
        GoTo SafeExit
    End If
    ...
    If Not Module4_ModelItemImporter.ApplyImportedCoverageToCandidates( _
        swApp, swView, candidates, horizontalDatum, verticalDatum, _
            evidence) Then
...
Private Function CollectOwnedCandidates( _
                    ...
                    If Not cylinderFace Is Nothing Then
                        ...
                        If Module3_ModelAudit.IsInternalCylindricalFace( _
                            ...
                            If IsArray(modelEdges) Then
                                ...
                                For edgeIndex = LBound(modelEdges) To _
                                    UBound(modelEdges)
    ...
    For acceptedIndex = 1 To results.Count
        ...
        evidence.AddInfo "Accepted candidate in '" & _
            ...
            "; modelAxis=" & Format$(acceptedCandidate.ModelAxisX, _
                "0.000000") & _
        ...
        evidence.AddInfo "EVIDENCE|CANDIDATE_ACCEPTED|view=" & _
            ...
            "|instance=" & _
                EvidenceValue(acceptedCandidate.PhysicalInstanceKey) & _
...
Private Function TryReadClosedCircularEdge( _
    ...
    If Not curveIsCircular Then
        ...
        ' CylinderParams-derived radius/axis cannot prove this edge is
        ' circular.
...
Private Function CurveParameterEndpointsAreCoincident( _
    ...
    If Module8_RuntimeSupport.CountVariantItems(startPoint) < 3 Or _
       Module8_RuntimeSupport.CountVariantItems(endPoint) < 3 Then Exit _
           Function
...
Private Function BuildOwnedCandidate( _
       ...
       featureTypeUpper <> "SKETCHHOLE" Then
        ...
        If InStr(featureTypeUpper, "CUT") = 0 Or _
           Not internalCylinderProven Then
            ...
            candidate.RejectionReason = _
                "GenericCutMatchedCylinderNotProvenInternal"
            Exit Function
        End If
    End If
    ...
    candidate.ModelAxisX = CDbl(circleData(circleBase + 3)) / _
        modelAxisMagnitude
    candidate.ModelAxisY = CDbl(circleData(circleBase + 4)) / _
        modelAxisMagnitude
    candidate.ModelAxisZ = CDbl(circleData(circleBase + 5)) / _
        modelAxisMagnitude
...
Private Sub CaptureFeatureSemantics( _
    ...
    Select Case UCase$(candidate.FeatureType)
        ...
        Case "ADVHOLEWZD"
            ...
            AppendSemanticProperty candidate, definition, _
                "NearSideElementsCount"
            AppendSemanticProperty candidate, definition, _
                "FarSideElementsCount"
    End Select
...
Private Function IsSameSemanticInstance( _
    ...
    If StrComp(firstCandidate.ComponentName, _
               secondCandidate.ComponentName, vbTextCompare) <> 0 Then Exit _
                   Function

    If StrComp(firstCandidate.ConfigurationName, _
               secondCandidate.ConfigurationName, vbTextCompare) <> 0 Then _
                   Exit Function

    If StrComp(firstCandidate.FeatureName, _
               secondCandidate.FeatureName, vbTextCompare) <> 0 Then Exit _
                   Function
    ...
    If Abs(axisDot) < Module8_RuntimeSupport.AXIS_NORMAL_MIN_COS Then Exit _
        Function
...
Private Function ProveDatum( _
    ...
    Select Case UCase$(Trim$(requestedOrigin))
        ...
        Case "CENTER"
            ProveCenterDatum swApp, swDrawModel, swView, candidates, proof, _
                evidence

        Case Else
            proof.FailureReason = "Unsupported datum option '" & _
                requestedOrigin & "'."
    End Select
End Function
...
Private Sub ProveCenterDatum( _
    ...
    ' The model origin is a reference point, not a claim about visible
    ' geometry.
    ' It legitimately projects outside the view outline whenever the part
    ' origin
    ...
    If Not Module8_RuntimeSupport.TransformPointToView( _
        ...
        proof.FailureReason = _
            "Projected model origin page frame was not proven: " & _
    ...
    For i = 1 To entities.Count
        If Abs(CDbl(xValues(i)) - projectedOriginX) <= _
           ...
           Module8_RuntimeSupport.PROJECTED_TOLERANCE_M Then
            ...
            proof.Proven = True
            proof.ProofSource = _
                "visible model vertex at projected model origin"
            proof.StableKey = BuildDatumStableKey(proof)
            Exit Sub
        End If
    Next i
    ...
    For i = 1 To candidates.Count
        ...
        If Abs(candidate.ViewX - projectedOriginX) <= _
           ...
           Module8_RuntimeSupport.PROJECTED_TOLERANCE_M Then
            ...
            proof.Proven = True
            proof.ProofSource = _
                "ownership-proven circular edge centered at projected " & _
                    "model origin"
            proof.StableKey = BuildDatumStableKey(proof)

            If proof.DrawingEntity Is Nothing Or proof.ModelEntity Is _
                Nothing Then
                proof.Proven = False
                proof.StableKey = vbNullString
                proof.FailureReason = _
                    "Circular datum edge/model correspondence does not " & _
                        "expose IEntity."
            End If
            Exit Sub
        End If
    ...
    proof.FailureReason = _
        "No selectable visible entity is exactly at the projected model " & _
            "origin."
    Exit Sub
        ...
        If Not alreadyCovered Then
            If Len(candidate.FamilyKey) = 0 Or _
               Len(candidate.PhysicalInstanceKey) = 0 Then
                ...
                evidence.AddFailure _
                    "Ordinate grouping received a candidate without a " & _
                        "stable family/instance key."
                Exit Function
            End If
    ...
    For i = 1 To groupCandidates.Count
        ...
        If coordinateMatches Then
            evidence.AddInfo "EVIDENCE|COORDINATE_SUPPRESSED|group=" & _
                ...
                EvidenceValue(candidate.PhysicalInstanceKey) & _
                "|coordinate=" & Format$( _
                    IIf(useX, candidate.ViewX, candidate.ViewY), _
                        "0.000000000") & _
                "|tolerance=" & _
...
Private Function CreateOneOrdinateGroup( _
    ...
    If selectableCandidates.Count = 0 Then
        evidence.AddInfo _
            "Ordinate direction contains only the proven zero entity in '" & _
            Module8_RuntimeSupport.GetViewName(swView) & "'."
        Exit Function
    End If
    ...
    If selectedByMulti <> selectableCandidates.Count Then
        evidence.AddFailure "MultiSelect2 selected " & _
            CStr(selectedByMulti) & _
            " of " & CStr(selectableCandidates.Count) & _
                " feature entities in '" & _
            Module8_RuntimeSupport.GetViewName(swView) & "'."
...
Failed:
    ' Capture first: GetViewName contains On Error Resume Next, which resets
    ' the global Err before either Err.Description or Err.Number is read.
    Dim groupErrorNumber As Long
    Dim groupErrorDescription As String
    groupErrorNumber = Err.Number
    groupErrorDescription = Err.Description

    evidence.AddFailure "Ordinate group error in '" & _
        Module8_RuntimeSupport.GetViewName(swView) & "': " & _
        groupErrorDescription
    outcome = "RuntimeError:" & CStr(groupErrorNumber)
    Resume SafeExit
End Function
