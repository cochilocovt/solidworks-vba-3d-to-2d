...
Public Sub PopulateTitleBlock( _
    ...
    If Not Module1_Main.GlobalConfig.PopulateTitle Then
        evidence.AddFailure _
            "Mandatory controlled title population was disabled."
        evidence.MarkStageFailed "TITLE_PROPERTIES", _
    ...
    If Len(partNumber) = 0 Then
        ...
        evidence.AddWarning _
            "Filename used only as visible part identification; " & _
    ...
    End If
    If Not WriteDrawingProperty( _
        drawingManager, "Description", description, descriptionSource, _
            evidence) Then
        titleStageValid = False
    End If
    If Not WriteDrawingProperty( _
    ...
    Else
        evidence.AddFailure _
            "Displayed scale cannot be sourced: sheet-scale readback is " & _
            "unproved."
        titleStageValid = False
    End If
    ...
    evidence.AddInfo _
        "Drawing properties were populated configuration-first; " & _
    ...
    If titleStageValid Then
        evidence.MarkStageProved "TITLE_PROPERTIES", _
            "mandatory values, property writes, exact links, rendered " & _
                "text, and title extents proved"
    Else
        evidence.MarkStageFailed "TITLE_PROPERTIES", _
...
SheetItems:
    If Module1_Main.GlobalConfig.InsertNotes Then
        ...
        If Len(generalNotes) = 0 Then
            ...
            evidence.AddInfo _
                "General notes sourced from the three manual references."
        End If
    ...
    If Module1_Main.GlobalConfig.InsertBarcode Then
        ...
        evidence.AddWarning _
            "Asterisk-delimited part identification was written " & _
        ...
        Else
            evidence.MarkStageFailed "PART_IDENTIFICATION", _
                "part-identification write, link, rendered value, or " & _
                    "extent failed"
        End If
    Else
...
Private Function EnsureAssociativeManufacturingCallout( _
    ...
    If Not RebuildDrawing( _
        swDrawModel, evidence, "P-0251 " & calloutKind & _
            " callout rebuild") Then
    ...
    If positionSet = False Then
        evidence.AddWarning _
            "IAnnotation.SetPosition2 returned False for the " & _
    ...
    If Not ManufacturingDefinitionExtentIsSafe( _
        swDraw, extentLeft, extentBottom, extentRight, extentTop, evidence) _
            Then
...
Private Function GetManufacturingCalloutTarget( _
    ...
    If Module8_RuntimeSupport.CountVariantItems(outline) <> 4 Then Exit _
        Function
...
Private Function ManufacturingDefinitionExtentIsSafe( _
    ...
    If extentLeft < evidence.ContentBorderLeft - tolerance Or _
       ...
       extentTop > evidence.ContentBorderTop + tolerance Then
        ...
        evidence.AddFailure _
            "P-0251 manufacturing-definition note crosses the " & _
    ...
    If RectanglesOverlapWithClearance( _
        ...
        evidence.AddFailure _
            "P-0251 manufacturing-definition note overlaps the " & _
                            ...
                            If CDbl(position(positionIndex)) >= _
                               ...
                               CDbl(position(positionIndex + 1)) <= _
                                ...
                                evidence.AddFailure "P-0251 manufacturing " & _
                                    "callout extent contains another " & _
                                    "annotation " & _
                                    "origin in '" & _
        Module8_RuntimeSupport.GetViewName(swView) & "'."
                                Exit Function
                ...
                If Not IsSameAnnotation( _
                    ...
                    If GetNoteExtent( _
                        ...
                        If RectanglesOverlapWithClearance( _
                            ...
                            evidence.AddFailure "P-0251 manufacturing " & _
                                "callout overlaps another measurable note " & _
                                    "extent."
                            Exit Function
                        End If
                    End If
...
Private Function ReadActiveConfigurationMassKg( _
    ...
    If status <> swMassPropertiesStatus_OK Then
        evidence.AddFailure _
            "Active-configuration mass calculation failed; " & _
    ...
    If Not IsArray(massProperties) Then
        evidence.AddFailure _
            "Active-configuration mass calculation returned " & _
    ...
    If UBound(massProperties) - baseIndex + 1 < 12 Then
        evidence.AddFailure _
            "Active-configuration mass-properties array is " & _
...
Private Function ReadFirstResolvedProperty( _
    ...
    For i = LBound(propertyNames) To UBound(propertyNames)
        ...
        If resultCode <> swCustomInfoGetResult_CachedValue And _
           resultCode <> swCustomInfoGetResult_ResolvedValue Then GoTo _
               NextProperty
...
Private Function VerifyLinkedText( _
    ...
    If Len(expectedText) = 0 Then
        ...
        If isRequired Then
            evidence.AddFailure _
                "Mandatory linked-note value is empty for '" & _
                propertyName & "'."
        End If
        Exit Function
    End If
    ...
    For i = 1 To notes.Count
        ...
        If NoteReferencesSemanticField( _
            ...
            ' provedExpectedText is what the semantic matcher actually
            ' accepted,
                ...
                If GetNoteExtent( _
                    ...
                    If NoteExtentIsInRegion( _
                        ...
                        evidence.AddTitlePropertyEvidence _
                            ...
                            isRequired, True
                        evidence.AddInfo _
                            "Controlled linked note proved property '" & _
    ...
    If isRequired Then
        evidence.AddFailure _
            "No exact visible linked note proved property '" & _
...
Private Function PrepareUniqueMassLink( _
    ...
    If matchCount = 0 Then
        evidence.AddFailure _
            "No unique property-linked mass note was found; " & _
    ...
    If StrComp(Trim$(repairedNote.PropertyLinkedText), _
        ...
        evidence.AddFailure _
            "Mass-note drawing-property link did not read back " & _
