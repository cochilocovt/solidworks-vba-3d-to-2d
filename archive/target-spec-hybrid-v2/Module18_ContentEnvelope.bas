...
' R23-908 remains the final answer: at most two measured rescale passes may
' run. If content still does not fit, the sheet is too small and that is the
' reported result. The cap prevents an uncontrolled shrinking loop.
...
' Text height does not scale with a drawing view. The 2026-08-04 scratch run
' proved a second measure-and-rescale pass was required after the geometric
' first estimate. Two passes are allowed; any third request fails closed.
Public Const MAX_RESCALE_PASSES As Long = 2
...
' MCP corpus value for swDrawingViewTypes_e.swDrawingDetailView.
...
' IView.UseParentScale is a COM Boolean. It is read before a mutation so a
' dependent view is not independently scaled as well as through its parent.
Private Function UsesParentScale( _
    ...
    UsesParentScale = Module11_GeometryIdentity.NormalizeSwBoolean( _
        swView.UseParentScale)
    known = True
    Exit Function

Failed:
    known = False
    UsesParentScale = False
End Function

' IView.UseSheetScale is an integer 1 or 0. The runner refuses to detach a
' view from the sheet when that relationship cannot first be read.
Private Function UsesSheetScale( _
    ByRef swView As SldWorks.View, _
    ByRef known As Boolean) As Boolean
    ...
    UsesSheetScale = (CDbl(swView.UseSheetScale) <> 0#)
    known = True
    Exit Function

Failed:
    known = False
    UsesSheetScale = False
End Function

' View names are unique within a drawing. This is used only to establish
' whether a dependent view's base is in the supplied layout set before any
' scale mutation starts.
Private Function CollectionHasViewName( _
    ...
    For i = 1 To views.Count
        ...
        If StrComp(SafeViewName(candidate), viewName, _
            vbTextCompare) = 0 Then

            CollectionHasViewName = True
            Exit Function
        End If
    Next i
End Function

' The user permits rescaling as a remedy, but the production pipeline still
' declares isometric and detail scales as approved presentation contracts.
' Do not silently override them to make a crowded sheet fit. A changed
' policy must be made deliberately at the view-creation contract too.
Private Function IsScaleLockedView( _
    ByRef swView As SldWorks.View, _
    ...
    If swView Is Nothing Then
        ...
        IsScaleLockedView = True
        Exit Function
    End If

    If swView.Type = VIEW_TYPE_DETAIL Then
        reason = "ApprovedDetailScale"
        IsScaleLockedView = True
        Exit Function
    End If
    ...
    If InStr(orientationName, "ISOMETRIC") > 0 Or _
        InStr(orientationName, "TRIMETRIC") > 0 Or _
        InStr(orientationName, "DIMETRIC") > 0 Then

        reason = "ApprovedIsometricScale"
        IsScaleLockedView = True
    End If
    Exit Function

Failed:
    reason = "ScaleLockReadError:" & CStr(Err.Number)
    IsScaleLockedView = True
End Function
...
Public Function MeasureSheetRegions( _
    ...
    ' The title block is reported, never invented. An unread or invalid one
    ' contributes no rectangle rather than a rectangle at the origin.
    ...
    If Not titleBlock Is Nothing Then
        ...
        Dim titleError As Long

        On Error Resume Next
        titleBlock.GetExtents titleX1, titleY1, titleX2, titleY2
        titleError = Err.Number
        Err.Clear
        On Error GoTo Failed

        If titleError <> 0 Then
            titleProof = "titleBlock=PresentBoundsUnread:" & _
                CStr(titleError)
        Else
            evidence.TitleBlockLeft = IIf(titleX1 < titleX2, _
                titleX1, titleX2)
            evidence.TitleBlockRight = IIf(titleX1 > titleX2, _
                titleX1, titleX2)
            evidence.TitleBlockBottom = IIf(titleY1 < titleY2, _
                titleY1, titleY2)
            evidence.TitleBlockTop = IIf(titleY1 > titleY2, _
                titleY1, titleY2)

            If IsRealRectangle(evidence.TitleBlockLeft, _
                evidence.TitleBlockBottom, evidence.TitleBlockRight, _
                evidence.TitleBlockTop) And _
                evidence.TitleBlockLeft >= 0# And _
                evidence.TitleBlockBottom >= 0# And _
                evidence.TitleBlockRight <= sheetWidth And _
                evidence.TitleBlockTop <= sheetHeight Then

                titleProof = "titleBlock=Measured" & _
                    "|source=ITitleBlock.GetExtents"
            Else
                evidence.TitleBlockLeft = 0#
                evidence.TitleBlockBottom = 0#
                evidence.TitleBlockRight = 0#
                evidence.TitleBlockTop = 0#
                titleProof = "titleBlock=PresentBoundsRejected"
            End If
        End If
    End If
...
Public Function PlanPlacement( _
    ...
    If viewEnvelopes Is Nothing Or viewEnvelopes.Count = 0 Then
        planProof = "plan=Reject|reason=NoViewEnvelopes"
        Exit Function
    End If
    ...
    If widestRow <= 0# Or totalHeight <= 0# Then
        planProof = "plan=Reject|reason=InvalidContentExtent"
        Exit Function
    End If
...
' MUTATES THE DRAWING. Multiplies independent root-view scales by one factor.
...
' took. Dependent views retain UseParentScale and are never scaled twice.
Public Function ApplyScaleToFit( _
    ...
    Dim applied As Long
    Dim detachedFromSheet As Long
    Dim inheritedViews As Long
    Dim readbackFailures As String

    ' Establish all relationships before touching one view. Otherwise a bad
    ' descendant would leave a sheet half-rescaled before its failure was
    ' known.
    Dim i As Long
    For i = 1 To views.Count
        Dim swView As SldWorks.View
        ...
        If IsScaleLockedView(swView, lockReason) Then
            readbackFailures = AppendFailure(readbackFailures, _
                SafeViewName(swView) & ":" & lockReason)
            GoTo ContinuePreflight
        End If

        Dim inherited As Boolean
        Dim parentKnown As Boolean
        inherited = UsesParentScale(swView, parentKnown)

        If Not parentKnown Then
            readbackFailures = AppendFailure(readbackFailures, _
                SafeViewName(swView) & ":UseParentScaleUnreadable")
            GoTo ContinuePreflight
        End If
        ...
        If inherited Then
            ...
            On Error Resume Next
            Set baseView = swView.GetBaseView
            On Error GoTo Failed

            If baseView Is Nothing Or _
                Not CollectionHasViewName(views, SafeViewName(baseView)) Then

                readbackFailures = AppendFailure(readbackFailures, _
                    SafeViewName(swView) & ":ParentNotInLayout")
            Else
                inheritedViews = inheritedViews + 1
            End If
        Else
            Dim sheetKnown As Boolean
            Call UsesSheetScale(swView, sheetKnown)
            If Not sheetKnown Then
                readbackFailures = AppendFailure(readbackFailures, _
                    SafeViewName(swView) & ":UseSheetScaleUnreadable")
            End If
        End If
...
ContinuePreflight:
    ...
    If Len(readbackFailures) > 0 Then
        ApplyScaleToFit = "rescale=Refused|reason=ScaleRelationship" & _
            "|relationshipFailures=" & readbackFailures
        Exit Function
    End If
    ...
    For i = 1 To views.Count
        Set swView = views(i)
        inherited = UsesParentScale(swView, parentKnown)
        If inherited Then GoTo ContinueScale

        Dim before As Double
        before = CDbl(swView.ScaleDecimal)

        Dim usesSheet As Boolean
        usesSheet = UsesSheetScale(swView, sheetKnown)
        If usesSheet Then
            evidence.RecordSolidWorksMutation _
                "IView.UseSheetScale:" & SafeViewName(swView)
            swView.UseSheetScale = 0

            If UsesSheetScale(swView, sheetKnown) Then
                readbackFailures = AppendFailure(readbackFailures, _
                    SafeViewName(swView) & ":UseSheetScaleStillTrue")
                GoTo ContinueScale
            End If

            detachedFromSheet = detachedFromSheet + 1
        End If
...
ContinueScale:
    ...
    ApplyScaleToFit = "rescale=Applied" & _
        ...
        "|inherited=" & CStr(inheritedViews) & _
        "|detachedFromSheet=" & CStr(detachedFromSheet) & _
...
Public Function ApplyPlacementPlan( _
    ...
    If views Is Nothing Then
        ApplyPlacementPlan = "layout=PlanMismatch|reason=ViewsUnavailable"
        Exit Function
    End If
    If viewEnvelopes Is Nothing Then
        ApplyPlacementPlan = _
            "layout=PlanMismatch|reason=EnvelopesUnavailable"
        Exit Function
    End If
    If views.Count <> viewEnvelopes.Count Then
        EmitFailure evidence, "LAYOUT_APPLY_REFUSED" & _
            "|reason=PlanViewCountMismatch" & _
            "|views=" & CStr(views.Count) & _
            "|envelopes=" & CStr(viewEnvelopes.Count)
        ApplyPlacementPlan = "layout=PlanMismatch"
        Exit Function
    End If

    ' Rescale before moving. Each factor is applied, rebuilt and re-measured
    ' rather than compounded arithmetically because annotation text does not
    ' shrink with a view. The bounded pass count remains fail-closed.
    ...
    If replan.Count <> views.Count Then
        EmitFailure evidence, "LAYOUT_APPLY_REFUSED" & _
            "|reason=PlanViewCountMismatch" & _
            "|views=" & CStr(views.Count) & _
            "|cells=" & CStr(replan.Count)
        ApplyPlacementPlan = "layout=PlanMismatch"
        Exit Function
    End If
    ...
    Dim rescalePass As Long
    Do While InStr(1, planProof, "plan=RescaleRequired", _
        vbBinaryCompare) > 0 And rescalePass < MAX_RESCALE_PASSES

        rescalePass = rescalePass + 1
        rescaleProof = ApplyScaleToFit(swDraw, views, suggestedFactor, _
            allowMutation, evidence)

        If InStr(1, rescaleProof, "rescale=Applied", _
            vbBinaryCompare) = 0 Then

            ApplyPlacementPlan = "layout=Reject|reason=RescaleFailed" & _
                "|rescalePasses=" & CStr(rescalePass) & _
                "|" & rescaleProof & _
        ...
        If replan.Count <> views.Count Then
            EmitFailure evidence, "LAYOUT_APPLY_REFUSED" & _
                "|reason=PostRescalePlanViewCountMismatch" & _
                "|views=" & CStr(views.Count) & _
                "|cells=" & CStr(replan.Count)
            ApplyPlacementPlan = "layout=PlanMismatch" & _
                "|reason=PostRescalePlanViewCountMismatch" & _
                "|" & ReportScaleChanges(views, capturedScales)
            Exit Function
        End If

        EmitInfo evidence, "LAYOUT_RESCALE|pass=" & CStr(rescalePass) & _
            "|" & rescaleProof & _
            "|" & planProof
    Loop
    ...
    ElseIf InStr(1, planProof, "plan=RescaleRequired", _
        vbBinaryCompare) > 0 Then

        ApplyPlacementPlan = "layout=Reject" & _
            "|reason=LargerSheetRequired" & _
            "|rescalePasses=" & CStr(rescalePass) & _
            "|" & rescaleProof & "|" & planProof & _
            "|" & ReportScaleChanges(views, capturedScales)
        Exit Function
    Else
        EmitFailure evidence, "LAYOUT_APPLY_REFUSED|reason=PlanRejected|" & _
            planProof
        ApplyPlacementPlan = "layout=Reject|reason=PlanRejected|" & _
            planProof
        Exit Function
    End If
...
Public Sub R23_ProbeContentEnvelope()
    ...
    If swApp Is Nothing Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=SolidWorksUnavailable"
        Exit Sub
    End If
    ...
    If swDraw Is Nothing Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=NoActiveDocument"
        Exit Sub
    End If

    If swDraw.GetType <> swDocDRAWING Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=ActiveDocumentNotDrawing"
        Exit Sub
    End If
    ...
    If IsEmpty(views) Or Not IsArray(views) Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=NoViewsOnSheet"
        Exit Sub
    End If
    ...
    If swPart Is Nothing Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=NoReferencedDocument"
        Exit Sub
    End If
    ...
    If Not Module1_Main.IsAuthorizedFixture(partPath) Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=UnauthorizedFixture" & _
            "|path=" & partPath
        Exit Sub
    End If
    ...
    Module21_EvidenceSink.LogLine _
        "R23_ENVELOPE_BEGIN|drawing=" & swDraw.GetPathName & _
        "|part=" & partPath & _
    ...
    If Not MeasureSheetRegions(swSheet, evidence, sheetProof) Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=SheetSizeUnmeasured" & _
            "|" & sheetProof
        Exit Sub
    End If

    Module21_EvidenceSink.LogLine _
        "QA INFO: ENVELOPE_SHEET" & _
    ...
    For i = LBound(views) To UBound(views)
        ...
        If IsTemplateOrientationView(swView) Then
            Module21_EvidenceSink.LogLine _
                "QA INFO: ENVELOPE_SKIPPED|view=" & _
...
ContinueView:
    ...
    If envelopes.Count = 0 Then
        Module21_EvidenceSink.LogLine _
            "R23_ENVELOPE_FATAL|reason=NoSeededEnvelopes"
        Exit Sub
    End If
    ...
    For p = 1 To protectedRegions.Count
        ...
        Set region = protectedRegions(p)
        Module21_EvidenceSink.LogLine _
            "QA INFO: PROTECTED|" & region.Summary()
    Next p
    ...
    Module21_EvidenceSink.LogLine _
        "QA INFO: ENVELOPE_CLEARANCE|" & clearanceVerdict
    ...
    Module21_EvidenceSink.LogLine _
        "QA INFO: ENVELOPE_PLAN|" & planProof
    ...
    Module21_EvidenceSink.LogLine _
        "R23_ENVELOPE_END|envelopes=" & CStr(envelopes.Count) & _
...
Failed:
    Module21_EvidenceSink.LogLine _
        "R23_ENVELOPE_FATAL|error=" & CStr(Err.Number) & _
