...
Public Function CreateNativeCalloutForFamily( _
    ...
    ' build and must not gate a transaction; activation is proved by
    ' active-view readback inside ActivateDrawingView.
    If projection.DrawingView Is Nothing Then
        EmitFailure evidence, "CALLOUT_CREATE_REFUSED" & _
            "|reason=ViewUnavailable|view=" & projection.ViewName
        Exit Function
    End If

    If Not Module8_RuntimeSupport.ActivateDrawingView( _
        swDraw, swDrawing, projection.DrawingView, evidence, _
        "Native callout in '" & projection.ViewName & "'") Then
...
Public Sub R23_ProbeCalloutDefinition()
    ...
    If swApp Is Nothing Then
        Module21_EvidenceSink.LogLine _
            "R23_CALLOUT_FATAL|reason=SolidWorksUnavailable"
        Exit Sub
    End If
    ...
    If swDraw Is Nothing Then
        Module21_EvidenceSink.LogLine _
            "R23_CALLOUT_FATAL|reason=NoActiveDocument"
        Exit Sub
    End If

    If swDraw.GetType <> swDocDRAWING Then
        Module21_EvidenceSink.LogLine _
            "R23_CALLOUT_FATAL|reason=ActiveDocumentNotDrawing"
        Exit Sub
    End If
    ...
    If IsEmpty(views) Or Not IsArray(views) Then
        Module21_EvidenceSink.LogLine _
            "R23_CALLOUT_FATAL|reason=NoViewsOnSheet"
        Exit Sub
    End If
    ...
    If swPart Is Nothing Then
        Module21_EvidenceSink.LogLine _
            "R23_CALLOUT_FATAL|reason=NoReferencedDocument"
        Exit Sub
    End If
    ...
    If Not Module1_Main.IsAuthorizedFixture(partPath) Then
        Module21_EvidenceSink.LogLine _
            "R23_CALLOUT_FATAL|reason=UnauthorizedFixture" & _
            "|path=" & partPath
        Exit Sub
    End If
    ...
    Module21_EvidenceSink.LogLine _
        "R23_CALLOUT_BEGIN|drawing=" & swDraw.GetPathName & _
        "|part=" & partPath & _
    ...
    If Not Module12_FeatureQualification.BuildFeatureCatalog( _
        ...
        Module21_EvidenceSink.LogLine _
            "R23_CALLOUT_FATAL|reason=CatalogUnavailable"
        Exit Sub
    End If
...
ContinueProjectionView:
    ...
    Module21_EvidenceSink.LogLine _
        "R23_CALLOUT_GRAPH|" & graph.GraphSummary()
    ...
    For i = LBound(views) To UBound(views)
        ...
        For d = LBound(dimensions) To UBound(dimensions)
            ...
            If Len(familyKey) = 0 Then
                Module21_EvidenceSink.LogLine _
                    "QA INFO: CALLOUT_UNATTRIBUTED|view=" & _
            ...
            Module21_EvidenceSink.LogLine _
                "QA INFO: CALLOUT_NATIVE|view=" & viewName & _
...
ContinueCalloutView:
    ...
    Module21_EvidenceSink.LogLine _
        "R23_CALLOUT_NATIVE_SUMMARY|nativeCallouts=" & _
        CStr(nativeCount) & _
    ...
    For f = 1 To familyKeys.Count
        ...
        Module21_EvidenceSink.LogLine _
            "QA INFO: CALLOUT_DEFINITION|" & decisionProof & _
            "|" & chosen.Summary()
...
ContinueFamily:
    ...
    Module21_EvidenceSink.LogLine _
        "R23_CALLOUT_END|" & definitionVerdict & _
        "|" & shapeVerdict & _
...
Failed:
    Module21_EvidenceSink.LogLine _
        "R23_CALLOUT_FATAL|error=" & CStr(Err.Number) & _
        "|description=" & Err.Description
