...
Public Sub R23_ProbeImportAllViews()
    R23_RunImportProbe "AllViewsTrue"
End Sub

Public Sub R23_ProbeImportSelectedViews()
    R23_RunImportProbe "SelectedViewsFalse"
End Sub

Private Sub R23_RunImportProbe(ByVal probeMode As String)
    On Error GoTo Failed
    ...
    If swApp Is Nothing Then
        Debug.Print "R23_IMPORT_PROBE_FATAL|reason=SolidWorksUnavailable"
        Exit Sub
    End If
    ...
    If swPart Is Nothing Then
        Debug.Print "R23_IMPORT_PROBE_FATAL|reason=NoActiveDocument"
        Exit Sub
    End If

    If swPart.GetType <> swDocPART Then
        Debug.Print "R23_IMPORT_PROBE_FATAL|reason=ActiveDocumentNotPart"
        Exit Sub
    End If
    ...
    If GetFixtureKey(partPath) <> "P-0251-14A-001" Then
        Debug.Print "R23_IMPORT_PROBE_FATAL" & _
            "|reason=AuthorizedP0251Required" & _
            "|path=" & R23_ProbeToken(partPath)
        Exit Sub
    End If
    ...
    InitializeEvidence swApp, swPart, partPath, templatePath
    GlobalEvidence.TemplatePath = templatePath

    Module4_ModelItemImporter.R23_ConfigureImportProbe probeMode

    Debug.Print "R23_IMPORT_PROBE_REQUEST_BEGIN" & _
        "|variant=" & R23_ProbeToken(probeMode) & _
        "|part=" & R23_ProbeToken(partPath)

    Module2_DrawingPipeline.CreateDrawing _
        swApp, swPart, partPath, templatePath, GlobalEvidence

    Debug.Print "R23_IMPORT_PROBE_REQUEST_END" & _
        "|variant=" & R23_ProbeToken(probeMode)
...
Failed:
    Debug.Print "R23_IMPORT_PROBE_FATAL|reason=UnhandledError" & _
...
Public Sub R23_InitializeProbeEvidence( _
    ByRef swApp As SldWorks.SldWorks, _
    ByRef swPart As SldWorks.ModelDoc2, _
    ByVal partPath As String, _
    ByVal templatePath As String)
