...
' R23 Phase 0 disposable-probe capture of the section-line construction.
' Each accepted segment appends its original page-frame endpoints and the
' converted view-sketch endpoints actually passed to CreateLine, so the probe
' module can log both coordinate frames without re-deriving them.
Public R23_SectionPlanCaptured As Boolean
Public R23_SectionPlanSourceView As String
Public R23_SectionPagePoints As Collection
Public R23_SectionSketchPoints As Collection

Public Sub R23_ResetSectionPlanCapture()
    R23_SectionPlanCaptured = False
    R23_SectionPlanSourceView = vbNullString
    Set R23_SectionPagePoints = New Collection
    Set R23_SectionSketchPoints = New Collection
End Sub

Private Sub R23_CaptureSectionPoint( _
    ByVal pageX As Double, _
    ByVal pageY As Double, _
    ByVal sketchX As Double, _
    ByVal sketchY As Double)

    On Error Resume Next
    If R23_SectionPagePoints Is Nothing Then Exit Sub

    Dim pagePoint(0 To 1) As Double
    Dim sketchPoint(0 To 1) As Double
    pagePoint(0) = pageX
    pagePoint(1) = pageY
    sketchPoint(0) = sketchX
    sketchPoint(1) = sketchY

    R23_SectionPagePoints.Add pagePoint
    R23_SectionSketchPoints.Add sketchPoint
    R23_SectionPlanCaptured = True
    On Error GoTo 0
End Sub
...
Public Sub RunDrawingPipeline( _
    ...
    If Module4_ModelItemImporter.R23_ImportProbeConfigured Then
        Module4_ModelItemImporter.R23_ExecuteConfiguredImportProbe _
            swApp, swPart, swDrawModel, swDraw, primaryView, evidence
        Exit Sub
    End If
...
Private Function CreatePrimarySection( _
    ...
    R23_ResetSectionPlanCapture
    R23_SectionPlanSourceView = _
...
Private Function AddSectionSegment( _
    ...
    R23_CaptureSectionPoint startX, startY, startViewX, startViewY
    R23_CaptureSectionPoint endX, endY, endViewX, endViewY
